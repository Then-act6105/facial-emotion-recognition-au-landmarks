# Facial Expression Recognition Using Generic and Action Unit-Aligned Facial Regions

A comparative study of CNN-based landmark processing and fusion strategies for facial expression recognition, conducted as an IEEE Computational Intelligence Society (CIS), Kolkata Chapter — Summer Internship 2026 project.

This repository implements and evaluates seven CNN architectures that differ **only** in how facial-region information is processed and fused, holding the convolutional backbone constant. The goal is to isolate the effect of landmark-region representation (generic vs. Action Unit-aligned) and fusion strategy (shared vs. landmark-specific backbones, feature-vector vs. feature-map fusion, early channel fusion, 3D convolution) on recognition performance — independent of any particular pretrained network family.

## Overview

Conventional CNN-based facial expression recognition (FER) processes the full face and must implicitly learn which regions are discriminative for each emotion. This project instead decomposes each face into either **7 generic facial-landmark regions** (eyes, eyebrows, nose, mouth, face contour) or **10 Action Unit (AU)-aligned regions** derived from the Facial Action Coding System (FACS), and systematically compares how different CNN architectures process and fuse these regional crops.
...
## Key Idea / Proposed Approach

Each face is decomposed into a fixed set of facial-region crops using MediaPipe Face Mesh landmarks (either 7 generic regions or 10 AU-aligned regions). A common **Standard CNN Block** (Conv → BatchNorm → ReLU → MaxPool, stacked 4×, channels 3→64→128→256→256) is used as the shared building block across all landmark-based architectures, so that differences in performance can be attributed to the fusion strategy rather than backbone capacity.

## Model Architecture

![Model Architecture](docs/architecture.png)

The diagram above shows the input processing pipeline and all seven evaluated model configurations.
...
- Data augmentation (random horizontal flip, rotation, color jitter) is available via `--augment`, but the paper's reported experiments were run **without** augmentation to isolate architectural effects.

### 7 Generic Landmark Regions
...
Derived from FACS: **AU1** (Inner Brow Raiser), **AU2** (Outer Brow Raiser), **AU4** (Brow Lowerer), **AU5** (Upper Lid Raiser), **AU6** (Cheek Raiser), **AU9** (Nose Wrinkler), **AU12** (Lip Corner Puller), **AU15** (Lip Corner Depressor), **AU17** (Chin Raiser), **AU26** (Jaw Drop). Each AU is mapped to a combination of MediaPipe landmark regions at runtime (`build_au_region_plan` / `build_action_units` in `src/Preprocessing/au_mapping.py`).
...
All results below are from the internship report (RAF-DB, seven emotion classes). **SCNN-FC** (Shared CNN with feature-vector fusion) is the primary/best-performing model in this study.
...
*7LM = 7 generic landmark regions; 10AU = 10 Action Unit-aligned regions. StandardCNN does not apply to the landmark-based 10AU configuration.*

### Model Complexity & Training Cost (RAF-DB / CK+, Parameters (M) / Avg. epoch time (s))
...
The landmark-extraction pipeline (`src/Preprocessing/landmarks.py`) loads a MediaPipe Face Landmarker model from a local file path (`face_landmarker.task` by default). This file is not bundled in the repository (see `.gitignore`) and must be downloaded separately from MediaPipe's model repository and placed at the path referenced in `landmarks.py`, or that path updated accordingly. If this file is not found, landmark extraction fails and the pipeline returns zero-valued patches.

## Dataset Setup

**Both Kaggle and local execution are supported by the codebase's directory conventions, but `src/Main/main.py` currently hardcodes a Kaggle input path.** To run outside Kaggle, this must be edited directly (there is no environment variable or config file for this — see *Recommended Improvements* below).

The dataset is expected in the following structure (matching the RAF-DB layout used on Kaggle):
...
**On Kaggle**, no changes are needed — `main.py` already points to:
```python
"dataset_path": "/kaggle/input/datasets/tanishmittal/rafdb"
```

**For local execution**, edit the following lines in `src/Main/main.py`:
```python
...
`IMG_DIR` is derived automatically from `dataset_path` as `<dataset_path>/dataset/aligned`, so updating `dataset_path` alone is sufficient once the CSV paths above are also corrected.
...
7-generic-landmark configuration (matches the paper's SCNN-FC best result):
```bash
...
- `Checkpoint_<landmark-id>_<model>_<dataset>.pt` — best model weights (by validation loss)
- `ExperimentLog_<landmark-id>_<model>_<dataset>.json` — full experiment log (system info, framework versions, dataset info, per-epoch losses/accuracies, timing, evaluation metrics)
- `Results_<landmark-id>_<model>_<dataset>.csv` — single-row summary of hyperparameters and final test metrics
...
- Training configuration used for the reported results: Adam, learning rate 0.01, `ReduceLROnPlateau`, batch size 64, up to 200 epochs, early stopping patience 10, no data augmentation.
...
- Python
- PyTorch, Torchvision
- MediaPipe (Face Mesh / Face Landmarker) for facial landmark detection
- scikit-learn (metrics, train/val/test splitting)
- Pandas, NumPy
- Matplotlib, Seaborn (plots and confusion matrix visualization)
- Pillow (image I/O)
...
> R. Majumdar, "Facial Expression Recognition Using Generic and Action Unit-Aligned Facial Regions: A Comparative Study of CNN-Based Landmark Processing and Fusion Strategies," Summer Internship Report, IEEE Computational Intelligence Society (CIS), Kolkata Chapter, 2026.

## Authors / Acknowledgements

**Students:** Rupam Majumdar, Kishalay Das — Dept. of Computer Science & Engineering, MCKV Institute of Engineering, Howrah, West Bengal, India

**Mentor:** Dr. Asit Barman, Asst. Prof., CSE/IT, Siliguri Institute of Technology, West Bengal, India

**Program:** IEEE Computational Intelligence Society (CIS), Kolkata Chapter — Summer Internship 2026 (1 June 2026 – 31 August 2026)
